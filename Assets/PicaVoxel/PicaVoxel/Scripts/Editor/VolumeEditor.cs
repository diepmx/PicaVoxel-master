...
namespace PicaVoxel
{
    ...
    public partial class VolumeEditor : Editor
    {
        ...
        private void DrawVoxelToolPanel()
        {
            ...
            // Paint Mode dropdown
            paintMode = (EditorPaintMode)EditorGUILayout.EnumPopup(
                voxelObject.PaintMode,
            ...
            // Value slider
            EditorPersistence.SelectedValue = (byte)GUILayout.HorizontalSlider(
                EditorPersistence.SelectedValue, 0f, 255f,
            ...
            string inputSelectedValue = EditorGUILayout.TextField(EditorPersistence.SelectedValue.ToString(),
                new GUIStyle(GUI.skin.textField) { padding = new RectOffset(2, 0, 2, 0), margin = new RectOffset(0, 0, 0, 5), fontStyle = FontStyle.Bold },
            ...
            // Color picker
            newColor = EditorGUILayout.ColorField(voxelObject.PaletteColors[EditorPersistence.SelectedColor], GUILayout.Width(90));
            ...
            // Tool buttons (giữ nguyên logic, không sửa)
            GUIToolButtons(skin);

            GUILayout.EndArea();
        }

        // --- Helper tách các nút tool mode để gọn code ---
        private void GUIToolButtons(GUISkin skin)
        {
            try
            {
                EditorCursorMode oldMode = EditorPersistence.CursorMode;
                GUILayout.BeginHorizontal();
                if (HighlightButton(EditorUtility.Buttons["pvButton_Add"], EditorPersistence.CursorMode == EditorCursorMode.Add, "Add single voxels"))
                {
                    buttonJustClicked = true;
                    EditorPersistence.CursorMode = EditorCursorMode.Add;
                }
                if (HighlightButton(EditorUtility.Buttons["pvButton_Subtract"], EditorPersistence.CursorMode == EditorCursorMode.Subtract, "Subtract single voxels"))
                {
                    buttonJustClicked = true;
                    EditorPersistence.CursorMode = EditorCursorMode.Subtract;
                }
                if (HighlightButton(EditorUtility.Buttons["pvButton_Paint"], EditorPersistence.CursorMode == EditorCursorMode.Paint, "Paint single voxels"))
                {
                    buttonJustClicked = true;
                ...
                GUILayout.EndHorizontal();
                GUILayout.BeginHorizontal();
                if (HighlightButton(EditorUtility.Buttons["pvButton_BoxAdd"], EditorPersistence.CursorMode == EditorCursorMode.BoxAdd, "Draw a box to add voxels"))
                {
                    buttonJustClicked = true;
                    EditorPersistence.CursorMode = EditorCursorMode.BoxAdd;
                }
                if (HighlightButton(EditorUtility.Buttons["pvButton_BoxSubtract"], EditorPersistence.CursorMode == EditorCursorMode.BoxSubtract, "Draw a box to subtract voxels"))
                {
                    buttonJustClicked = true;
                    EditorPersistence.CursorMode = EditorCursorMode.BoxSubtract;
                }
                if (HighlightButton(EditorUtility.Buttons["pvButton_BoxPaint"], EditorPersistence.CursorMode == EditorCursorMode.BoxPaint, "Draw a box to paint voxels"))
                {
                    buttonJustClicked = true;
                ...
                GUILayout.EndHorizontal();
                GUILayout.BeginHorizontal();
                if (HighlightButton(EditorUtility.Buttons["pvButton_Select"], EditorPersistence.CursorMode == EditorCursorMode.Select, "Select voxels to create a brush"))
                {
                    buttonJustClicked = true;
                    EditorPersistence.CursorMode = EditorCursorMode.Select;
                }
                if (HighlightButton(EditorUtility.Buttons["pvButton_PickColor"], EditorPersistence.CursorMode == EditorCursorMode.PickColor, "Change the currently selected color to the color of an existing voxel"))
                {
                    buttonJustClicked = true;
                    EditorPersistence.CursorMode = EditorCursorMode.PickColor;
                }
                if (HighlightButton(EditorUtility.Buttons["pvButton_PickValue"], EditorPersistence.CursorMode == EditorCursorMode.PickValue, "Change the currently selected value to the value of an existing voxel"))
                {
                    buttonJustClicked = true;
                ...
                GUILayout.EndHorizontal();
                GUILayout.BeginHorizontal();
                if (HighlightButton(EditorUtility.Buttons["pvButton_BrushAdd"], EditorPersistence.CursorMode == EditorCursorMode.BrushAdd, "Add voxels using stored brush"))
                {
                    buttonJustClicked = true;
                    EditorPersistence.CursorMode = EditorCursorMode.BrushAdd;
                }
                if (HighlightButton(EditorUtility.Buttons["pvButton_BrushSubtract"], EditorPersistence.CursorMode == EditorCursorMode.BrushSubtract, "Subtract voxels using stored brush"))
                {
                    buttonJustClicked = true;
                    EditorPersistence.CursorMode = EditorCursorMode.BrushSubtract;
                }
                if (HighlightButton(EditorUtility.Buttons["pvButton_BrushPaint"], EditorPersistence.CursorMode == EditorCursorMode.BrushPaint, "Paint voxels using stored brush"))
                {
                    buttonJustClicked = true;
                ...
                // ... Các tool khác phía dưới (Selection, Brush, v.v) copy logic từ OnSceneGUI cũ xuống đây nếu muốn luôn hiện trong tool panel ...
            }
            catch (Exception) { }
        }
        ...
        {   
            ...
            voxelSizeProperty = so.FindProperty("VoxelSize");
            overlapAmountProperty = so.FindProperty("OverlapAmount");
            collisionMode = so.FindProperty("CollisionMode");
            meshingMode = so.FindProperty("MeshingMode");
            meshCompression = so.FindProperty("MeshCompression");
            colliderMeshingMode = so.FindProperty("MeshColliderMeshingMode");
            separateColliderMesh = so.FindProperty("GenerateMeshColliderSeparately");
            pivotProperty = so.FindProperty("Pivot");

            voxelSize = voxelSizeProperty.floatValue;
            overlapAmount = overlapAmountProperty.floatValue;
            pivot = pivotProperty.vector3Value;

            selfShadeInt = so.FindProperty("SelfShadingIntensity");

            runtimeOnlyMesh = so.FindProperty("RuntimeOnlyMesh");

            material = so.FindProperty("Material");
            physicMaterial = so.FindProperty("PhysicMaterial");
            castShadows = so.FindProperty("CastShadows");
            receiveShadows = so.FindProperty("ReceiveShadows");
            chunkLayer = so.FindProperty("ChunkLayer");
            collisionTrigger = so.FindProperty("CollisionTrigger");

            isEnabledForEditing = so.FindProperty("IsEnabledForEditing");
        ...
        {
            // --- Đảm bảo menu tool luôn hiển thị ---
            DrawVoxelToolPanel();

            // Phần còn lại giữ nguyên logic cũ, chỉ bỏ phần panel tool, giữ lại animation và thao tác trên scene:
            // Ví dụ:
            ...
            // ... Giữ nguyên logic animation panel, handles, v.v. ở phía dưới hàm ...
            // Animation control vẫn để như code cũ
            // ...

            Handles.SetCamera(Camera.current);
            DoCursor(Event.current);
        }
